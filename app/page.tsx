...
export default function Home() {
  ...
  // Accent color used across the surface
  const brandColor = "#00D991"
  const gridBackgroundStyle = React.useMemo<React.CSSProperties>(() => {
    const gridColor = "rgba(255,255,255,0.06)"
    const white = "rgba(255, 255, 255, 0.08)"
    return {
      backgroundImage: [
        `radial-gradient(1000px 700px at 100% 100%, ${white}, transparent 60%)`,
        `repeating-linear-gradient(0deg, ${gridColor} 0, ${gridColor} 1px, transparent 1px, transparent 48px)`,
        `repeating-linear-gradient(90deg, ${gridColor} 0, ${gridColor} 1px, transparent 1px, transparent 48px)`
      ].join(','),
        ...
        <AnimatePresence mode="wait">
          {!started && (
            <motion.div
              ...
              className="relative z-10 w-full border border-neutral-800/60 rounded-xl overflow-hidden min-h-[32rem] md:min-h-[40rem]"
            >
              <div className="grid grid-cols-1 md:grid-cols-[30%_70%] bg-neutral-900">
              {/* Left visual panel */}
              <div className="relative hidden md:block" style={gridBackgroundStyle}>
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <div className="text-xl font-medium tracking-tight flex items-center gap-2">
                    ...
                    >
                      <svg viewBox="0 0 78 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="transition-all duration-200" style={{ height: '0.85em', width: 'auto', overflow: 'visible', display: 'block' }}>
              ...
              {/* Right info panel */}
              <div className="relative p-8 md:p-10 bg-[#111111] flex flex-col">
                <div className="text-3xl md:text-4xl font-semibold leading-tight">How to run this project</div>
                <div className="mt-3 text-sm text-neutral-400 space-y-4">
                  <div>
                    <div className="text-sm font-medium text-neutral-200">Prereqs</div>
                    <ul className="list-disc pl-5 mt-1 space-y-1">
                      <li>Node 18+ and Yarn</li>
                      <li>A Kalshi API Key ID and Private Key</li>
                    </ul>
                  </div>

                  <div>
                    <div className="text-sm font-medium text-neutral-200">Install</div>
                    <div className="mt-2 rounded-md border border-neutral-800 bg-neutral-900 p-3 text-xs text-neutral-200">
                      <pre className="whitespace-pre-wrap break-words">yarn install</pre>
                    </div>
                  </div>

                  <div>
                    <div className="text-sm font-medium text-neutral-200">Environment</div>
                    <p className="mt-1">Create <span className="font-mono text-neutral-300">.env.local</span> in the project root:</p>
                    <div className="mt-2 rounded-md border border-neutral-800 bg-neutral-900 p-3 text-xs text-neutral-200">
...
KALSHI_KEY=...
                    ...
                    <ul className="list-disc pl-5 mt-2 space-y-1">
                      <li><span className="font-mono">PROD_KEY_ID</span>: Your Kalshi API Key ID.</li>
                      <li><span className="font-mono">KALSHI_KEY</span>: The private key. Paste as one line with <span className="font-mono">\n</span> or the full multi-line PEM. Both work.</li>
                    </ul>
                  </div>

                  <div>
                    <div className="text-sm font-medium text-neutral-200">Get a Kalshi API key</div>
                    <ul className="list-disc pl-5 mt-1 space-y-1">
                      <li>On Kalshi: <span className="font-mono">Account → Security → API Keys → Create Key</span></li>
                      <li>Save your <span className="font-mono">API Key ID</span> and download the <span className="font-mono">Private Key</span> (.key)</li>
                    </ul>
                    <div className="mt-3 rounded-md border border-neutral-800 bg-neutral-900 p-3 text-xs text-neutral-300">
                      Put the ID in <span className="font-mono">PROD_KEY_ID</span> and the private key in <span className="font-mono">KALSHI_KEY</span> (single line with \n or the full multi-line PEM).
                    </div>
                    <div className="mt-3 rounded-md border border-neutral-800 bg-neutral-900 p-3 text-xs text-neutral-300">
                      This app is read-only. It fetches your portfolio data and does not place or cancel orders. See <span className="font-mono">app/api/wrapped</span> and <span className="font-mono">app/api/portfolio</span>.
                    </div>
                  </div>

                  <div>
                    <div className="text-sm font-medium text-neutral-200">Run</div>
                    <div className="mt-2 rounded-md border border-neutral-800 bg-neutral-900 p-3 text-xs text-neutral-200">
                      <pre className="whitespace-pre-wrap break-words">yarn dev</pre>
                    </div>
                    <div className="mt-1 text-xs text-neutral-500">Open http://localhost:3000</div>
                  </div>
                </div>
                <div className="mt-8 md:mt-auto flex justify-end">
                  <Button className="cursor-pointer" onClick={() => setStarted(true)}>Start</Button>
                </div>
              </div>
              </div>
            </motion.div>
        ...
        {started && (
          <Wrapped triggerClassName="hidden" triggerLabel="" autoStart onStart={() => {}} />
        )}
      </div>
    </div>
  );
